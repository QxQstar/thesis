<!DOCTYPE html>
<html>
<head lang="en">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0">
    <title>没有权限</title>
    <link type="text/css" rel="stylesheet" href="/thesis/src/user/build/css/common.css">
</head>
<body>
<div class="g-main f-show f-main-bg" style="margin-top: 0">
    <div class="m-limit f-text-c f-marTop-60">
        <img src="/thesis/src/user/build/img/noquestion.png">
        <h1 style="color: #666">你没有设置安全问题，不能找回密码</h1>
        <div class="f-marTop-20">
            <a href="/thesis/src/PHP/show/index.php" style="color: #ff3300;text-decoration: underline;">继续看看</a>
            <a href="/thesis/src/PHP/show/signin.php" style="color: #ff3300;text-decoration: underline;">立即注册</a>
        </div>

    </div>
</div>
</body>
</html>